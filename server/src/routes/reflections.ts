...
reflectionsRouter.put("/:moduleNum", async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ error: "Invalid reflection", details: parsed.error.flatten() });
...
reflectionsRouter.post("/:moduleNum/complete", async (req, res) => {
  const moduleNum = Number(req.params.moduleNum);
  if (!Number.isInteger(moduleNum) || moduleNum < 1 || moduleNum > 12) {
    return res.status(400).json({ error: "moduleNum must be 1-12" });
  }
...
reflectionsRouter.get("/completions", async (_req, res) => {
